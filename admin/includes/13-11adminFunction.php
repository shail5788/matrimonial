...
class AdminFunction{
    ...
    function addStaticContent($args,$id){
        $data =  mysqli_num_rows($this->con->query('SELECT * FROM tbl_static_data where category_id = "'.$args['category'].'"'));
        if($data == 0){
...
public function addPlan($args){
  ...
  public function planList(){
    ...
    public function cityDetail($id){
      $queryData =  $this->con->query('SELECT * FROM `tbl_city` where id = "'.$id.'"');
    ...
    public function cityList(){
       $i = 1;
        ...
        while($detail =$data->fetch_assoc()){
            $html.='<tr><td>'.$i.'</td>
            <td>'.$detail['city'].'</td>
            <td><p id="'.$detail['id'].'" class="btn btn-danger">Delete</p><a href="editCity.php?id='.$detail['id'].'" class="btn btn-primary">Edit</a></td>';
            $i = $i+1;
   ...
   $updateData =  $this->con->query('update tbl_city set city= "'.$args['city'].'" where id = "'.$id.'"');
   return 1;
         }
  ...
  public function ReligionList(){
       $i = 1;
        ...
        while($detail =$data->fetch_assoc()){
            ...
            <td><p id="'.$detail['id'].'" class="btn btn-danger">Delete</p><a href="editReligion.php?id='.$detail['id'].'" class="btn btn-primary">Edit</a></td>';
            $i = $i+1;
