<?php
  ...
  class ApiClass{
      ...
      public function get_allProfessional($pID){
          $response=$this->db->select('tbl_professional',['prof_id'=>$pID]);
          return $response;
      }

      public function get_profession($pID){
          $response=$this->db->select('tbl_profession',['id'=>$pID]);
          return $response;
      }

       public function get_professionalName($pID){
          $response=$this->db->select('tbl_professional',['id'=>$pID]);
          return $response;
      }

      public function get_country_name($cID){
          $response=$this->db->select('countries',['id'=>$cID]);
          return $response;
      }

      public function get_state_name($sID){
          $response=$this->db->select('states',['id'=>$sID]);
          return $response;
      }

      public function get_city_name($cID){
          $response=$this->db->select('tbl_city',['id'=>$cID]);
          return $response;
      }
     ...
     public function get_edu_category(){
      ...
      public function get_professional(){
          ...
          $data=$this->db->fetch_all("tbl_profession");
          return $data;
     ...
     public function get_plan(){
      ...
      // code for desired-partner   
      public function save_match_partner_basic($data){
            
            $user_id=$data['user_id'];
            
            if(!empty($this->is_exist('desired_partner',$user_id))){
              
                $response=$this->db->row_update("desired_partner",$data,['user_id'=>$user_id]);

                if($response){

                   $this->message['status_code']=200;
                   $this->message['message']="Basic info Update!";
                   $this->message['response']=true;
                   ...
                   $this->message['response']=fales;
                }

            }else{
              
               $response=$this->db->insert("desired_partner",$data);
               if($response){

                   $this->message['status_code']=200;
                   $this->message['message']="Basic info inserted!";
                   $this->message['response']=true;
      ...
      public function save_desire_p_education($data){
            ...
            if(!empty($this->is_exist('desired_partner',$user_id))){
                ...
                if($response){
                   ...
                   $this->message['status_code']=200;
                   $this->message['message']="Education info Update!";
                   $this->message['response']=true;
               ...
               if($response){
                   ...
                   $this->message['status_code']=200;
                  $this->message['message']="Education info inserted!";
                   $this->message['response']=true;
      ...
      public function get_desired_partner_info($user_id){
             
           $data=$this->db->select("desired_partner",['user_id'=>$user_id]);

            $data[0]['country']=$this->db->select("countries",['id'=>$data[0]['country']]);

           $data[0]['state']=$this->db->select("states",['id'=>$data[0]['state']]);

           $data[0]['highest_qualification']=$this->db->select("tbl_education",['id'=>$data[0]['highest_qualification']]);

           $data[0]['occupation']=$this->db->select("tbl_professional",['id'=>$data[0]['occupation']]);
      ...
      public function save_about_partner($data){
     ...
     public function is_exist($table,$user_id){

          $response=$this->db->select($table,['user_id'=>$user_id]);
          return $response;
     }
     public function get_education(){

        $response=$this->db->fetch_all('tbl_education');
        return $response;

     }
     public function get_occupation(){

       $response =$this->db->fetch_all("tbl_professional");
       return $response; 
     }
  }
