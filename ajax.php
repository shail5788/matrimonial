  ...
  if($_POST['action']=="resettingPassword"){
   ...
   if($_POST['action']=="feedbackSave"){
    ...
    if($_POST['action']=="searchFilter"){
       ...
       $marital_status=$_POST['marital_status'];exit;
      ...
      if($startage!='' && $endage !='')
      { 
          $where .="AND  FLOOR(DATEDIFF (NOW(), dob)/365) BETWEEN '".$startage."' AND '".$endage."' ";
      }
      if($country !='')
      {
      ...
      if($religion !='')
      {
         ...
         // echo "select tbl_user.*,FLOOR(DATEDIFF (NOW(), dob)/365)  AS act_age from tbl_user ".$where;exit;
       
      $selectData = $mysqli->query("select tbl_user.*,TIMESTAMPDIFF(YEAR, dob, CURDATE()) AS age  from tbl_user ".$where);
         $data=[];
